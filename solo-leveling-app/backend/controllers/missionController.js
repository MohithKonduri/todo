...
const { DAILY_QUEST_POOL, INSTANT_QUEST_POOL } = require('../gameData/quests');

const getRandomQuests = (pool, count) => pool.sort(() => 0.5 - Math.random()).slice(0, count);
...
exports.getDailyMissions = async (req, res) => {
    try {
        ...
        await user.save();
        const hasSystemQuests = await Mission.countDocuments({ user: req.user.id, date: today, source: 'System' });
        if (hasSystemQuests === 0) {
            const missionsToCreate = [];
            missionsToCreate.push(...getRandomQuests(DAILY_QUEST_POOL.Common, 3));
            missionsToCreate.push(...getRandomQuests(DAILY_QUEST_POOL.Uncommon, 1));
            if (Math.random() < 0.2) missionsToCreate.push(...getRandomQuests(DAILY_QUEST_POOL.Rare, 1));
            if (Math.random() < 0.05) missionsToCreate.push(...getRandomQuests(INSTANT_QUEST_POOL, 1));
            const systemMissions = missionsToCreate.map(m => ({ ...m, user: req.user.id, date: today, source: 'System' }));
            await Mission.insertMany(systemMissions);
        }
